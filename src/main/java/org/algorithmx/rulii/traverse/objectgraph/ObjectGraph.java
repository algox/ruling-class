...
public class ObjectGraph {
    ...
    public void traverse(Object target, AnnotatedTypeDefinition typeDefinition, ObjectVisitor visitor) throws ObjectGraphTraversalException {
        Assert.notNull(visitor, "visitor cannot be null.");
        candidates.clear();
        breadCrumbs.clear();
        // Add the root object
        addCandidate(new TraversalCandidate(target, typeDefinition), visitor);
    ...
    private void traverseObject(TraversalCandidate candidate, ObjectVisitor visitor) throws IllegalAccessException {
        ...
        boolean introspectObject = visitor.visitObjectStart(candidate);
        //if (introspectObject) processFields(target, visitor);
        visitor.visitObjectEnd(candidate);

    }

    private void addCandidate(TraversalCandidate candidate, ObjectVisitor visitor) {
